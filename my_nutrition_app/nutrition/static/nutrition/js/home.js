(function(){
  /*
   File: nutrition/static/nutrition/js/home.js
   Apraksts: Galvenā klienta puses loģika lapai "home" — loģika, kas
   atbild par ierakstu sarakstu, skaitļu parsēšanu, kopējām summām,
   rediģēšanu inline, un produktu pievienošanu no lokālā/API avota.
   Kur atrodas: iekļauts šablonā [nutrition/templates/nutrition/home.html](nutrition/templates/nutrition/home.html).
   Mērķis: nodrošināt klienta interaktivitāti (AJAX pieprasījumi, priekšskatījumi,
   UI atjauninājumi) bez pilnas lapas pārlādes.
   Piezīme: Šis fails izmanto globālas utilītfunkcijas no `common.js`.
  */

  // Globālie atkarības/konfigurācija
  ...
  // gettext fallback — ja Django jsi18n nav pievienots, nodrošina drošu noklusējumu
  const gettext = window.gettext || function(s){ return s; };
  const ngettext = window.ngettext || function(s, p, n){ return n === 1 ? s : p; };

  // Neliels utilīta palīgs — parsē skaitli no elementa tekstsatura vai no data-* atribūta.
  // Kāpēc: HTML var saturēt vērtības gan redzamā tekstā, gan `data-*` atribūtos (kad DOM nav atjaunināts),
  // tāpēc šī funkcija nodrošina vienotu veidu iegūt granulāras skaitliskas vērtības.
  ...
  // --- addToTotals(delta) ---
  // Ko dara: atjaunina augšējo kopējo bloku (kopējās kcal, proteīns, tauki, ogļhidrāti)
  // un arī 'eat progress' joslu, ja tā ir lapā. Saņem deltas (var būt negatīvas) un
  // pievieno tās esošajām summām.
  // Kāpēc: servera atbildes vai lokālas izmaiņas (rediģēšana, dzēšana) maina kopējās vērtības;
  // šī funkcija centralizē atjaunināšanu, lai izvairītos no dublikācijas un saglabātu pieejamību visiem skriptiem.
  ...
  // Ko dara: droši paslēpj inline rediģēšanas formu. Ja formā bija fokuss, pārvieto fokusu uz rediģēšanas pogu
  // vai izvērtē blur, lai nepieļautu, ka fokusēts elements paliek neredzams (pieejamības un UX iemeslu dēļ).
  ...
  // Ko dara: pielieto servera atbildē norādītās autoritatīvās vērtības konkrētam ierakstam DOM sarakstā.
  //  - atjaunina `data-*` atribūtus, lai nākotnē lasīšana būtu konsekventa
  //  - atjaunina redzamo tekstu rindā
  //  - aprēķina deltas un nodod tās `addToTotals`, lai atjauninātu kopējās summas
  // Kāpēc: serveris var koriģēt aprēķinus vai validēt ierakstus — klientam jāspēj sinhronizēt DOM ar servera stāvokli
  ...
  // Šis bloks: 1) atjaunina priekšskatījumu, kad lietotājs aizpilda pielāgotu produktu formu,
  // 2) validē ievadi un 3) nosūta POST uz API (`/nutrition/api/add-entry/`) ar CSRF galveni.
  // Kāpēc: ļauj ātri pievienot lietotāja definētus produktus bez manuālas tabulas ievades serverī.
  (function(){
    ...
    submitBtn.addEventListener('click', async function(){
      ...
      const name = (nameEl.value || '').trim();
      const amt = parseLocaleNumber(amountEl.value);
      if(!name){ msg.textContent = gettext('Enter product name'); return; }
      if(!amt || amt <= 0){ msg.textContent = gettext('Enter valid amount'); return; }
      ...
      if(kcal < 0 || protein < 0 || fat < 0 || carbs < 0){
        msg.textContent = gettext('Enter non-negative nutrient values'); return;
      }
      if(kcal === 0 && protein === 0 && fat === 0 && carbs === 0){
        if(!confirm(gettext('All nutrient fields are zero. Continue?'))) return;
      }
      ...
      try {
        ...
        if(resp.status === 401 || resp.status === 403){
          msg.textContent = gettext('Please log in to save entries.');
          return;
        }
        if(!resp.ok){
          ...
          if(ct.includes('application/json')){
            ...
            msg.textContent = (err && (err.error || err.message)) ? String(err.error || err.message) : gettext('Add failed');
          } else {
            msg.textContent = gettext('Add failed (server)');
          }
          return;
        }
        ...
        msg.textContent = gettext('Network error, try again.');
      }
    });
  })();

  // --- API meklēšana / svītrkodu lookup / rezultātu renderēšana ---
  // Šis bloks atbild par:
  //  - meklēšanu ārējā produktu datubāzē (API),
  //  - svītrkoda meklēšanu (barcode lookup),
  //  - rezultātu attēlošanu kā kartes un pievienošanas pogu.
  // Kāpēc: nodrošina ātru produktu atrodšanu un pievienošanu lietotāja dienasgrāmatai bez lapas pārslodzes.
  (function(){
    ...
    async function searchProducts(q){
      ...
      apiResults.innerHTML = '<p style="color:#666;">' + escapeHtml(gettext('Searching...')) + '</p>';
      try {
        ...
        if(!results.length){ apiResults.innerHTML = '<p style="color:#666;">' + escapeHtml(gettext('No results.')) + '</p>'; return; }
        renderResults(results);
      } catch(e){ console.warn('searchProducts error', e); showMessage(gettext('External database error. Try again or add product manually.'), 'error'); }
    }

    // renderResults(items)
    // Ko dara: veido vizuālas kartes no rezultātu saraksta, parāda makro/kaloriju datus
    // un pievieno notikumus pogai "Add from API". Izmanto `escapeHtml` un `parseLocaleNumber`
    // lai izvairītos no XSS un pareizi apstrādātu lokalizētus skaitļus.
    function renderResults(items){
      ...
      items.forEach(it=>{
        ...
        card.innerHTML = `
          <strong style="display:block; margin-bottom:0.5rem;">${escapeHtml(it.name || ('(' + gettext('unnamed') + ')'))}</strong>
          <div style="font-size:0.95rem; color:#444;">
            <div>${escapeHtml(gettext('Calories'))}: <b>${Number(kcal100||0).toFixed(1)}</b> kcal/100g</div>
            <div>${escapeHtml(gettext('Protein'))}: <b>${Number(prot100).toFixed(1)}</b> g/100g</div>
            <div>${escapeHtml(gettext('Fat'))}: <b>${Number(fat100).toFixed(1)}</b> g/100g</div>
            <div>${escapeHtml(gettext('Carbs'))}: <b>${Number(carbs100).toFixed(1)}</b> g/100g</div>
          </div>
          <div style="margin-top:0.75rem; display:flex; gap:0.5rem; align-items:center;">
            <input type="text" inputmode="decimal" value="100" class="amount-input" style="width:110px; padding:0.4rem; border-radius:6px; border:1px solid var(--border-color);">
              <button class="btn btn-primary add-api-btn">${escapeHtml(gettext('Add from API'))}</button>
          </div>
        `;
        const amountInput = card.querySelector('.amount-input');
        const btn = card.querySelector('.add-api-btn');
        // Poga: pievieno izvēlēto API produktu kā ierakstu
        //  - aprēķina gala kcal/proporcijas pēc ievadītā daudzuma
        //  - nosūta POST uz API ar CSRF galveni
        //  - pēc panākuma pārslādē lapu (vai paziņo par kļūdu)
        btn.addEventListener('click', async function(){
          const amount = parseLocaleNumber(amountInput.value);
          if(amount <= 0){ alert(gettext('Enter a positive amount (grams)')); return; }
          let original = it;
          ...
          try {
            ...
            if(!resp.ok){ alert(gettext('Add failed')); return; }
            const data = await resp.json().catch(()=>null);
            if(data && data.success) location.reload(); else alert(gettext('Add failed'));
          } catch(e){ console.error('add entry error', e); alert(gettext('Network error, try again.')); }
        });
    ...
    // lookupBarcode(code)
    // Ko dara: meklē produktu pēc svītrkoda ārējā API. Ja atrasts — parāda to `renderResults`.
    // Kāpēc: dažiem produktiem lietotājam ir svītrkods, tas nodrošina ātru pievienošanu.
    async function lookupBarcode(code){
      const url = (URLS.api_product_lookup || '/nutrition/api/product-lookup/') + '?barcode=' + encodeURIComponent(code);
      apiResults.innerHTML = '<p style="color:#666;">' + escapeHtml(gettext('Looking up barcode...')) + '</p>';
      try {
        const res = await fetch(url, { method:'GET', headers:{ 'Accept':'application/json' }, credentials:'same-origin' });
        const text = await res.text();
        const json = text ? JSON.parse(text) : null;
        if(!res.ok){ if(res.status === 404) showMessage(gettext('Product not found for this barcode.'), 'error'); else showMessage(gettext('Lookup failed, try again.'), 'error'); return; }
        const result = json && (json.result || json.item || json.data) ? (json.result || json.item || json.data) : json;
        if(result) renderResults([result]); else showMessage(gettext('Product not found for this barcode.'), 'error');
      } catch (e){ console.error('lookupBarcode error', e); showMessage(gettext('Lookup failed, try again.'), 'error'); }
    }
  })();

  // --- "sticker" (ieraksta) darbības: toggle-edit, delete un inline submit ---
  // Šis bloks klausās globālos klikšķus un nosūta atbilstošas darbības:
  //  - toggle-edit: atver/slēdz ieraksta rediģēšanas formu (pieejamības fokusēšana tiek apstrādāta)
  //  - delete: veic dzēšanas POST pieprasījumu (ja ir `delete` forma) vai lokālu dzēšanu
  //  - submit (rediģēšana): nosūta jauno daudzumu serverim un sinhronizē DOM ar servera atbildi
  (function(){
    function readEntryValues(li){
      const kcal = _parseTextOrData(li, '.entry-kcal', 'kcal');
      const protein = _parseTextOrData(li, '.entry-protein', 'protein');
    ...
    // Globālais klikšķu apstrādātājs — centralizē darbību atpazīšanu pēc `data-action`.
    // Risinājums izvairās no pievienošanas daudziem atsevišķiem klausītājiem katram ierakstam.
    document.addEventListener('click', function(e){
      ...
      if(action === 'delete'){
        if(!confirm(gettext('Are you sure you want to delete this entry?'))) return;
          ...
          fetch(actionUrl, {
            ...
            if(ct.includes('application/json')){
              ...
              if(data && data.success){
                ...
                li.remove();
                try{ localStorage.setItem('nutrition:entries-updated', String(Date.now())); }catch(_){ }
              } else alert(data && data.error ? data.error : gettext('Delete failed'));
              return;
            }
            if(resp.ok){
              if(typeof addToTotals === 'function') addToTotals({ kcal: -vals.kcal, protein: -vals.protein, fat: -vals.fat, carbs: -vals.carbs });
              ...
              const t = await resp.text(); console.error('Delete failed:', t); alert(gettext('Delete failed (server)'));
            }
          }).catch(err=>{ console.error('Delete request failed', err); alert(gettext('Network error, try again.')); });
          return;
        }
    ...
    // Inline rediģēšanas forma: apstrāde `submit` notikumam
    //  - validē jauno daudzumu
    //  - aprēķina jaunos kcal/proporcijas, ja nepieciešams
    //  - ja forma norāda `action`, nosūta POST uz serveri un izmanto servera atbildi
    //  - ja nav `action`, atjaunina DOM lokāli
    document.addEventListener('submit', async function(e){
      ...
      if(!newAmount || newAmount <= 0){ alert(gettext('Enter valid amount')); return; }
      ...
      // Ja formai ir `action`, ceram uz servera atbildi un prioritāti servera vērtībām.
      if(action && action.trim() !== ''){
        fetch(action, {
          ...
          const ct = resp.headers.get('content-type') || '';
          // Ja serveris atgriež JSON, izmanto autoritatīvās vērtības un `applyServerUpdateToListItem`.
          if(ct.includes('application/json')){
            const data = await resp.json();
            if(data && data.success){
              ...
              return;
               ...
               alert((data && (data.error || data.message)) ? (data.error || data.message) : gettext('Edit failed'));
               return;
            }
          }
          // Fallback: ja serveris neatgriež JSON, izmanto lokālo aprēķinu, bet atzīmē kopējās summas.
          if(resp.ok){
            ...
            const t = await resp.text(); console.error('Edit failed', t); alert(gettext('Edit failed'));
          }
        }).catch(err=>{ console.error('Edit request failed', err); alert(gettext('Network error, try again.')); });
        return;
      }

      // Local-only update (nav servera action): atjaunina DOM un kopējās summas tūlītēji
      if(oldAmountEl) oldAmountEl.textContent = newAmount.toFixed(1) + 'g';
      if(kcalEl) kcalEl.textContent = newKcal.toFixed(2);
      if(pEl) pEl.textContent = newP.toFixed(2);
      if(fEl) fEl.textContent = newF.toFixed(2);
  ...
  // initEatProgress
  // Ko dara: inicializē "eat progress" joslu pie lapas ielādes — nolasa `data-eaten` un `data-rec`,
  // aprēķina % un iestata stila/aria atribūtus. Parāda saprotamus tekstus, ja ieteikuma nav.
