...
def signup(request):
    """
    User registration page: accepts POST with username/email/password1/password2.
    Creates User and Profile, then redirects to home after login.
    """
    if request.method == 'POST':
        form = SignUpForm(request.POST)
        if form.is_valid():
            user = form.save()
            # Create associated Profile (OneToOne)
        ...
        form = SignUpForm()
    return render(request, 'nutrition/signup.html', {'form': form})
...
def api_add_entry(request):
    """
    Accept JSON body:
    { name, kcal, protein, fat, carbs, amount }
    Create Product (if not exists) and FoodEntry then return success JSON.
    """
    try:
    ...
    try:
        kcal = float(payload.get('kcal') or 0)
    ...
    # update nutrition values if product newly created or has zero values
    changed = False
    if created:
        product.calories_per_100g = kcal
        product.protein_per_100g = protein
        product.fat_per_100g = fat
        product.carbs_per_100g = carbs
        changed = True
    else:
        # update if fields missing/zero to keep product usable
        if (not product.calories_per_100g) and kcal:
            product.calories_per_100g = kcal
            changed = True
        if (not product.protein_per_100g) and protein:
            product.protein_per_100g = protein
            changed = True
        if (not product.fat_per_100g) and fat:
            product.fat_per_100g = fat
            changed = True
        if (not product.carbs_per_100g) and carbs:
            product.carbs_per_100g = carbs
            changed = True
    if changed:
        product.save()
    ...
    entry = FoodEntry.objects.create(product=product, amount=amount, initial_amount=amount)
    return JsonResponse({
        'success': True,
        'entry_id': entry.id,
        'calories': entry.calories if hasattr(entry, 'calories') else None
    })
